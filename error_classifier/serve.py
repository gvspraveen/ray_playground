...
system_content = """
You are an error type classifier for error SUMMARY. Classify the ERROR_TYPE of the SUMMARY as one of PYTHON_CODE_ISSUE or MISSING_DEPENDENCY or NODE_FAILURE. generate a valid JSON array of response, the response should ONLY INCLUDE JSON and it should be parsed in its ENTIRETY each error should contain the following properties:
1. REASON: This is step by step followed by you to determine the classification
2. ERROR_TYPE: This is ERROR_TYPE classification based on the REASON above
...
REASON: Let's think step-by-step. Summary does not refer to any failed nodes or termination issues. It is also not talking about a missing import or dependency. Instead summary suggests the code was not correctly implemented. ERROR_TYPE: PYTHON_CODE_ISSUE

SUMMARY: Your environment does not have 'gputil' installed which is required for GPU system monitoring. pip install gputil
REASON: Let's think step-by-step. The summary states that a library needed to run the code is not present. Also there is a suggestion to install a library. This is a case of missing dependency. ERROR_TYPE: MISSING_DEPENDENCY
...
REASON: Let's think step-by-step. The summary states that a Ray actor has failed. Even though it refers to Ray, it is clear that it failure is due to node failing or terminated and nothing to do with usage of Ray. ERROR_TYPE: NODE_FAILURE
